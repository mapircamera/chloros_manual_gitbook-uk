...
### Мінімальна площа калібрувальної вибірки (px)

* **Тип**: Число
* **Діапазон**: від 0 до 10 000 пікселів
* **За замовчуванням**: 25 пікселів
* **Опис**: Встановлює мінімальну площу (в пікселях), необхідну для того, щоб виявлена область вважалася дійсною зразком калібрувальної мішені. Менші значення дозволяють виявляти менші мішені, але можуть збільшити кількість помилкових спрацьовувань. Більші значення вимагають більших і чіткіших областей мішеней для виявлення.
* **Коли коригувати**:
...
* **Опис**: Вибирає алгоритм демозаїкування, який використовується для перетворення необроблених даних датчика за схемою Байєра у повнокольорові зображення. Метод «Стандартний (швидкий, середня якість)» забезпечує оптимальний баланс між швидкістю обробки та якістю зображення. Метод «З урахуванням текстури (повільний, найвища якість)» \[Chloros+] використовує високоякісний дебайер з урахуванням країв у поєднанні з моделлю шумозаглушення AI/ML, яка усуває майже весь шум дебайрингу. Для роботи моделі з урахуванням текстури потрібна пам&#x27;ять GPU (VRAM). Ми рекомендуємо використовувати її, якщо у вас є &gt;4 ГБ VRAM для швидшої обробки.
...
* **Опис**: Встановлює мінімальний інтервал часу (у секундах) між використанням калібрувальних мішеней. Якщо встановлено значення 0, Chloros використовуватиме кожну виявлену калібрувальну мітку. Якщо встановлено вище значення, Chloros використовуватиме лише калібрувальні мітки, віддалені одна від одної щонайменше на цю кількість секунд, що скорочує час обробки наборів даних із частими знімками калібрувальних міток.
* **Коли коригувати**:
  * Встановіть значення 0 для максимальної точності калібрування при зміні умов освітлення.
...
* **Коли регулювати**: Встановіть значення, що відповідає зміщенню вашого місцевого часового поясу, якщо ваші дані PPK використовують місцевий час замість UTC. Наприклад:
...
* **Опис**: Увімкнення використання постоброблених кінематичних (PPK) корекцій із реєстраторів MAPIR DAQ, що містять GPS (GNSS). Якщо увімкнено, Chloros використовуватиме будь-які файли журналу .daq, що містять дані виставлення експозиції у вашому каталозі проєкту, та застосовуватиме точні корекції геолокації до ваших зображень.
...
* **Тип**: Вибір із випадаючого меню
* **Видимість**: Видно тільки тоді, коли увімкнено «Застосувати корекції PPK» І доступні дані експозиції для контакту 1
* **Опції**:
...
* **Опис**: Призначає конкретну камеру до контакту експозиції 1 для синхронізації часу PPK. Контакт експозиції фіксує точний час спрацьовування затвора камери, що є критично важливим для точної геолокації PPK.
...
3. **Перевірте перед повним обробленням**: експериментуючи з новими налаштуваннями, перевірте їх на невеликій підгрупі зображень, перш ніж обробляти весь набір даних.
