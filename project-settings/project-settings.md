...
2. Клацніть на вкладку **Налаштування проекту**  <img src="../.gitbook/assets/icon_project-settings.JPG" alt="" data-size="line"> на лівій бічній панелі
...
* **За замовчуванням**: 60
* **Опис**: Контролює поріг кластеризації для групування подібних кольорових областей під час виявлення цілей калібрування. Більші значення вимагають групування більш подібних кольорів, що призводить до більш консервативного виявлення цілей. Менші значення дозволяють більшу варіацію кольорів у межах групи цілей.
* **Коли слід регулювати**:
...
* **Тип**: прапорець
* **За замовчуванням**: увімкнено (позначено)
* **Опис**: Увімкнення автоматичного калібрування відбиття за допомогою виявлених калібрувальних мішеней у ваших зображеннях. Це нормалізує значення відбиття у всьому наборі даних і забезпечує стабільність вимірювань незалежно від умов освітлення.
...
* **Опис**: Вибирає алгоритм демозаїкування, який використовується для перетворення необроблених даних датчика за схемою Байєра у повнокольорові зображення. Метод «Висока якість (швидше)» забезпечує оптимальний баланс між швидкістю обробки та якістю зображення.
...
* **Опис**: Встановлює мінімальний інтервал часу (у секундах) між використанням калібрувальних мішеней. При встановленні значення 0 Chloros буде використовувати кожну виявлену калібрувальну мішень. При встановленні більш високого значення Chloros буде використовувати тільки калібрувальні мішені, які розділені принаймні такою кількістю секунд, що скорочує час обробки наборів даних із частими зйомками калібрувальних мішеней.
* **Коли коригувати**:
...
* **Коли коригувати**: встановіть це значення відповідно до вашого місцевого часового поясу, якщо ваші дані PPK використовують місцевий час замість UTC. Наприклад:
...
* **Тип**: прапорець
* **За замовчуванням**: вимкнено (невідмічено)
* **Опис**: увімкнення використання корекцій Post-Processed Kinematic (PPK) з реєстраторів MAPIR DAQ, що містять GPS (GNSS). Якщо ця опція увімкнена, Chloros використовуватиме будь-які файли журналу .daq, що містять дані експозиційного контакту у вашому проектному каталозі, і застосовуватиме точні корекції геолокації до ваших зображень.
* **Вимога**: файл журналу .daq із записами експозиційного контакту повинен бути присутнім у вашому проектному каталозі
* **Коли увімкнути**: рекомендується завжди увімкнути корекцію PPK, якщо у вашому файлі журналу .daq є записи зворотного зв&#x27;язку експозиції.

### Контакт експозиції 1

* **Тип**: Вибір із випадаючого меню
* **Видимість**: Видно тільки тоді, коли ввімкнено «Застосувати корекції PPK» І доступні дані експозиції для контакту 1
* **Опції**:
...
* **Опис**: Призначає конкретну камеру для контакту експозиції 1 для синхронізації часу PPK. Контакт експозиції фіксує точний час спрацьовування затвора камери, що є критично важливим для точної геолокації PPK.
...
* **Тип**: Спеціальна панель конфігурації індексу
...
* **Опис**: Дозволяє створювати та зберігати власні формули мультиспектральних індексів за допомогою математичних операцій з діапазонами. Власні формули зберігаються разом із налаштуваннями проекту і можуть використовуватися так само, як і вбудовані індекси.
* **Як створити**:
  1. У панелі налаштувань індексу знайдіть опцію власних формул
  2. Визначте формулу, використовуючи ідентифікатори смуг (наприклад, NIR, Red, Green, Blue)
...
* **Опції**:
  ...
  * **PNG (8-бітний)** - Стислий 8-бітний формат PNG
...
* **Рекомендації щодо формату**:
  ...
  * **PNG (8-біт)**: Підходить для перегляду в Інтернеті та загальної візуалізації. Менший розмір файлів завдяки стисненню без втрат, але зменшений динамічний діапазон.
  * **JPG (8-біт)**: Найменший розмір файлів, найкраще підходить лише для попереднього перегляду та веб-відображення. Використовує стиснення з втратами, яке не підходить для наукового аналізу.
...
* **Приклади використання**:
  * Створіть шаблони для різних систем камер (RGB, мультиспектральна, NIR)
  * Збережіть стандартні конфігурації для конкретних типів культур або робочих процесів аналізу
  * Поділіться єдиними налаштуваннями з усією командою
* **Як використовувати**:
...
Усі налаштування проекту автоматично зберігаються разом із файлом проекту (формат проекту `.mapir`). Коли ви знову відкриваєте проект, усі налаштування відновлюються точно так, як ви їх залишили.
...
Більшість змін налаштувань (особливо в категоріях «Обробка» та «Експорт») спричинять повторну обробку зображень, щоб відобразити нові налаштування. Однак деякі налаштування є «тільки для експорту» і не вимагають негайної повторної обробки:
...
3. **Виконайте тестування перед повним обробленням**: експериментуючи з новими налаштуваннями, протестуйте їх на невеликій підгрупі зображень, перш ніж обробляти весь набір даних.
4. **Документуйте налаштування**: використовуйте описові назви шаблонів, що вказують на систему камер, тип обробки та передбачуване використання (наприклад, «Survey3\_RGB\_NDVI\_Agriculture»).
5. **Вибір формату експорту**: виберіть формат експорту відповідно до кінцевого використання:
   ...
   * Спільний доступ в Інтернеті → JPG (8-біт)
