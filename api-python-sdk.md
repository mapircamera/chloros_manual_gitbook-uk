...
**Chloros Python SDK** забезпечує програмний доступ до механізму обробки зображень Chloros, що дозволяє автоматизувати процеси, налаштовувати робочі процеси та безперешкодно інтегруватись із вашими додатками Python та дослідницькими конвеєрами.
...
**Повертає:** `dict` — відповідь про створення проекту
...
| `recursive`   | bool     | Ні       | Пошук у підпапках (за замовчуванням: False) |
...
| `indices`                 | список | `None`                  | Індекси рослинності для обчислення |
...
* `"TIFF (16-bit)"` — рекомендується для ГІС/фотограмметрії
...
### Приклад 5: Налаштування моніторингу прогресу
...
2. Перевірте, чи Windows брандмауер не блокує
3. Спробуйте вручну ввести шлях до бекенду:
...
2. **Зменште роздільну здатність виводу** (якщо це прийнятно)
...
### З: Чи потрібно для SDK підключення до Інтернету?
...
| Функція         | Desktop GUI | CLI Command Line | Python SDK  |
| --------------- | ----------- | ---------------- | ----------- |
| **Інтерфейс**   | Покажчик миші | Команди          | Python API  |
...
Зверніться до info@mapir.camera для отримання інформації про OEM.
