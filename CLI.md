...
2. Виконайте вказівки майстра встановлення
...
| --------------------- | ------- | -------------- | -------------------------------------------------------------------------------------- |
| `<input-folder>`      | Шлях    | _Обов&#x27;язковий_     | Папка, що містить мультиспектральні зображення RAW/JPG                                         |
| `-o, --output`        | Шлях    | Такий самий, як вхідний  | Папка виводу для оброблених зображень                                                     |
| `-n, --project-name`  | Строка  | Автоматично згенерована | Ім&#x27;я проекту, задане користувачем                                                                    |
...
### `export-status` — перевірка прогресу експорту

Моніторинг прогресу експорту потоку 4 під час або після обробки.
...
| --------------- | ------- | ------------- | ------------------------------------------------ |
| `--backend-exe` | Шлях    | Автоматичне виявлення | Шлях до виконуваного файлу бекенду                       |
| `--port`        | Ціле число | 5000          | Номер порту бекенду API                          |
| `--restart`     | Прапорець    | -             | Примусовий перезапуск бекенду (завершує існуючі процеси) |
...
**Що робить:** Коректує падіння освітлення на краях зображення (темніші кути, типові для зображень з камери).
